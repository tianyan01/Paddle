...
#pragma unroll
    ...
    if (threadIdx.x % WARP_SIZE == 0) {
      paddle::platform::CudaAtomicAdd(number_count + i, x);
    }
  }
}
...
void NumberCountKernel(const Context& dev_ctx,
  ...
  auto out_dims = make_ddim({upper_range});
  ...
  NumberCount<T>
      <<<CEIL(upper_range, PERTHREAD_EXPERTS), PERTHREAD_EXPERTS, 0, dev_ctx.stream()>>>(
          gate_data, out_data, batch_size, upper_range);
}

}  // namespace phi

PD_REGISTER_KERNEL(number_count_v2,
                   GPU,
                   ALL_LAYOUT,
                   phi::NumberCountKernel,
                   int64_t) {}