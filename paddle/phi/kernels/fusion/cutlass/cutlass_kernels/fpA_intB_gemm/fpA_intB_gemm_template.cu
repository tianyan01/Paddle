  ...
  switch (gemm_config.tile_config) {
    ...
    case CutlassTileConfig::CtaShape128x128x64_WarpShape128x32x64:
      if (arch::kMinComputeCapability < 75) {
        throw std::runtime_error("Invalid config on Volta");
      } else {
        dispatch_gemm_config<T,
                             WeightType,
                             arch,
...
void CutlassFpAIntBGemmRunner<T, WeightType>::run_gemm<EpilogueTag>(
  ...
  int64_t key = static_cast<int64_t>((static_cast<int64_t>(m) << 44) |
                                     static_cast<int64_t>(n) << 22 | k);
  ...
  if (it == config_cache_.end()) {
    ...
    // Standard GEMM, so 1 "expert". We use the same function for MoE and
    // regular FFN.
