...
CommonHead = '''
// Generated by generic_moe_gemm_kernelLauncher.py - Do not edit.
...
template<>
void generic_moe_gemm_kernelLauncher_template<{T},
                                                {WeightType},
                                                {arch},
                                                {EpilogueTag},
                                                {ThreadblockShape},
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="The argument for generating the generic_moe_gemm_kernelLauncher instance."
    )
    parser.add_argument(
        "--cuda_arch",
...
if __name__ == "__main__":
    ...
    if archs:
        for element_type in ElementTypes.keys():
            for weight_type in WeightTypes[element_type]:
                for arch in archs:
                    for epilogue_tag in EpilogueTags.keys():
                        for stages in StagesList[arch]:
                            new_type = weight_type.replace("cutlass::", "")
                            file_name = "autogen/generic_moe_gemm_kernelLauncher_{}_{}_sm{}_stages{}_{}.cu".format(
                                element_type, new_type, arch, stages, epilogue_tag
                            )
                            all_code = generate_source_cu(
                                element_type, weight_type, arch, epilogue_tag, stages
                            )
