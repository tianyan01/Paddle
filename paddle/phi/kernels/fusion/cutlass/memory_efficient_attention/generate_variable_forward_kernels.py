...
void  {NAME}({CPP_CLASS} default_fmha, Params &params, const phi::GPUContext& ctx) {{
  ...
  auto problem_sizes_device0 = paddle::memory::Alloc(
      ctx.GetPlace(),
      problem_count * sizeof(GemmCoord),
      phi::Stream(reinterpret_cast<phi::StreamId>(ctx.stream())));
  auto problem_sizes_device1 = paddle::memory::Alloc(
      ctx.GetPlace(),
      problem_count * sizeof(GemmCoord),
      phi::Stream(reinterpret_cast<phi::StreamId>(ctx.stream())));
  ...
  if (AttentionKernel::kNeedsOutputAccumulatorBuffer) {{
    ...
    auto tmp_output_accum_buffer_ptr = paddle::memory::Alloc(
        ctx.GetPlace(),
