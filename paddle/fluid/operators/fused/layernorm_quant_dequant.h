...
#pragma unroll
    ...
    ComputeType row_variance = max(row_sum_square_mean - row_mean * row_mean, static_cast<ComputeType>(0.0));
    ComputeType row_inv_var = Rsqrt(row_variance + static_cast<ComputeType>(epsilon));
    if (threadIdx.x == 0) {
      mean[row] = row_mean;
...
inline cudaError_t TryDispatchLayerNormBlockSMemImplBlockSize(
    cudaStream_t stream, LOAD load, STORE store, const int64_t rows, const int64_t cols,
    const double epsilon, ComputeType* mean, ComputeType* inv_variance, ComputeType col_divisor, bool* success) {
  ...
  if (max_active_blocks_conf_4 == max_active_blocks_conf_1
    ...
    *success = true;
    return LaunchLayerNormBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size_conf_4>(
        stream, load, store, smem, rows, cols, epsilon, mean, inv_variance, col_divisor);
  }
...
struct TryDispatchLayerNormBlockSMemImplPackSize {
  cudaError_t operator()(cudaStream_t stream, LOAD load, STORE store, const int64_t rows,
                         const int64_t cols, const double epsilon, ComputeType* mean,
                         ComputeType* inv_variance, 
                         ComputeType col_divisor, bool* success) {
    if (cols % 4 == 0 && CanPackAs<LOAD>(load, 4) && CanPackAs<STORE>(store, 4)) {
      return TryDispatchLayerNormBlockSMemImplBlockSize<LOAD, STORE, ComputeType, 4>(
          stream, load, store, rows, cols, epsilon, mean, inv_variance, col_divisor, success);
  ...
  if (cols <= 1024) {
    ...
    bool dispatch_smem_impl_success;
    {
      cudaError_t err = TryDispatchLayerNormBlockSMemImpl<LOAD, STORE, ComputeType>(
          stream, load, store, rows, cols, epsilon, mean, inv_variance, col_divisor, 
...
struct DequantSkipLoad {
  using LoadType = DST;
  DequantSkipLoad(const InputType* src, const SRC* bias, const SRC* skip, const float* dequant_scale, float alpha, int64_t row_size)
      : src(src), bias(bias), skip(skip), dequant_scale(dequant_scale), alpha(alpha), row_size(row_size) {}
  template<int N>
  __device__ void load(DST* dst, int64_t row, int64_t col) const {
...
#pragma unroll
    for (int i = 0; i < N; ++i) {
      ...
      dst[i] = static_cast<DST>(static_cast<DST>(static_cast<float>(src_pack.elem[i]) * dequant_scale_pack.elem[i])
                                + bias_pack.elem[i]
                                + skip_pack.elem[i]);
    }
  }
...
template<typename OutType, typename SRC, typename DST, bool do_scale, bool do_center>
struct AffineQuantStore {
  AffineQuantStore(OutType* y, 
              const int64_t row_size, 
              const float* gamma, const float* beta, 
              const float quant_out_scale = 1.0, 
              const int quant_round_type = 1,
              const float quant_max_bound = 127.0,
              const float quant_min_bound = -127.0) : y(y), row_size(row_size), gamma(gamma), beta(beta), quant_round_type(quant_round_type), 
                                                  quant_out_scale(quant_out_scale), quant_max_bound(quant_max_bound), quant_min_bound(quant_min_bound) {}
  ...
  __device__ void store(const SRC* src, int64_t row, int64_t col) {
    ...
    gamma_pack.storage = *(reinterpret_cast<const PackType<float, N>*>(gamma) + gamma_offset);
    beta_pack.storage = *(reinterpret_cast<const PackType<float, N>*>(beta) + gamma_offset);
#pragma unroll
    for (int i = 0; i < N; ++i) {
      float normalized_i = static_cast<float>(src[i]);
      float normalized_val = normalized_i * gamma_pack.elem[i] + beta_pack.elem[i];
      if (do_scale) {
        y_pack.elem[i] = QuantHelperFunc<float, OutType>(normalized_val, quant_out_scale, quant_round_type, quant_max_bound, quant_min_bound); 
      } else {
        y_pack.elem[i] = static_cast<OutType>(normalized_val);
      }
    }
...
template<typename InputType, typename SRC, typename DST, bool do_dequant>
struct DequantSkipLoadAndStoreResidual {
  ...
  DequantSkipLoadAndStoreResidual(const InputType* src, 
                          ...
                          float alpha, int64_t row_size)
      : src(src), bias(bias), skip(skip), dequant_scale(dequant_scale), residual_bias_out(residual_bias_out), alpha(alpha), row_size(row_size) {}
  template<int N>
  __device__ void load(DST* dst, int64_t row, int64_t col) const {
    Pack<InputType, N> src_pack;
    Pack<SRC, N> bias_pack;
...
#pragma unroll
    for (int i = 0; i < N; ++i) {
      // First we need to cast src and dequant.
      if (do_dequant) {
        residual_out_pack.elem[i] = static_cast<DST>(static_cast<DST>(static_cast<float>(src_pack.elem[i]) * dequant_scale_pack.elem[i])
                                                    + bias_pack.elem[i]
                                                    + skip_pack.elem[i]);
      } else {
...
#pragma unroll
    ...
    *(reinterpret_cast<PackType<SRC, N>*>(residual_bias_out) + offset) = residual_out_pack.storage;
  }
  const InputType* src;
  const SRC* bias;
  const SRC* skip;
