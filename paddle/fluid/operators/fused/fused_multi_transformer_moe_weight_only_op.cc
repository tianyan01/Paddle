...
class FusedMultiTransformerMoeWeightOnlyOp
    : public framework::OperatorWithKernel {
 private:
  static constexpr const char *OpName = "FusedMultiTransformerMoeWeightOnlyOp";
...
#define CHECK_OUTPUTS(name) \
  OP_INOUT_CHECK(ctx->HasOutputs(#name), "Output", #name, OpName)
    ...
    const std::string act_method = ctx->Attrs().Get<std::string>("act_method");
    PADDLE_ENFORCE(
    ...
    if (ctx->HasInputs("CacheKV")) {
      ...
      PADDLE_ENFORCE_EQ(c_dim[4],
                        ...
                        paddle::platform::errors::InvalidArgument(
                            ...
                            "size %d, but got %d, cache dims: %s, weight dims: %s",
                            head_size,
                            c_dim[4], c_dim, y_dim));  // head_size
    }
...
class FusedMultiTransformerMoeWeightOnlyOpOpMaker
 ...
 public:
  void Make() override {
    ...
    AddInput("OutLinearBias", "The out_linear bias tensor.")
        .AsDispensable()
        .AsDuplicable();
    AddInput("GateWeight", "The gate_weights in moe").AsDuplicable();
    AddInput("GateBias", "The gate_biases in moe").AsDuplicable();
    ...
    AddInput("FFNLnBias", "The layer_norm bias of FusedFeedForward op")
        .AsDuplicable();
    AddInput("ExpertWeight1", "The expert_weights1 in moe").AsDuplicable();
    AddInput("ExpertWeight1Scale", "The expert_weights1 scale in moe")
        .AsDuplicable();
    AddInput("ExpertBias1", "The expert_biases1 in moe").AsDuplicable();
    AddInput("ExpertWeight2", "The expert_weights2 in moe").AsDuplicable();
    AddInput("ExpertWeight2Scale", "The expert_weights2 scale in moe")
        .AsDuplicable();
    AddInput("ExpertBias2", "The expert_biases2 in moe").AsDuplicable();
    ...
    AddAttr<int>(
        ...
        .SetDefault(-1);
    // for moe layer
    AddAttr<int>("topk", "gate's topk im moe").SetDefault(2);
    AddAttr<int>("mp_size", "mp size").SetDefault(1);
    AddAttr<int>("mp_rank", "mp rank").SetDefault(0);
    AddAttr<int>("num_expert", "experts num im moe").SetDefault(1);
    AddAttr<int>("world_size", "world size").SetDefault(1);
    AddAttr<int>("moe_ring_id", "experts communicate group's ring id")
        .SetDefault(1);
    AddAttr<bool>("approximate", "approximate in expert compute gelu")
        .SetDefault(true);
...
REGISTER_OPERATOR(
    ...
    ops::FusedMultiTransformerMoeWeightOnlyOp,
    ops::FusedMultiTransformerMoeWeightOnlyOpOpMaker,
